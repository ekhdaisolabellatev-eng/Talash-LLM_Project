...
class Milestone2Analysis:
    ...
    def _normalize_skill(self, skill):
        """
        Normalize a skill entry to a string.
        ...
        if isinstance(skill, str):
            ...
            return str(skill_name).strip().lower() if skill_name else ''
        else:
    ...
    def analyze_educational_profile(self):
        ...
        def parse_year(record):
            try:
                ...
                year = record.get('graduation_year') or record.get('passing_year') or record.get('year')
                return int(year)
        ...
        for i in range(1, len(education_sorted)):
            ...
            if gap > 2:
                current_degree = education_sorted[i].get('degree_name') or education_sorted[i].get('degree', 'current degree')
                previous_degree = education_sorted[i - 1].get('degree_name') or education_sorted[i - 1].get('degree', 'previous degree')
                gaps.append(
                    f"A {gap}-year gap found between {previous_degree} and {current_degree}."
                )
        ...
        ranked_institutions = [
            ...
            if (
                ...
                or str(record.get('institution_name') or record.get('institution', '')).strip().lower() in known_ranked_institutions
            )
        ]

        reliable_records = [
            record for record in education_sorted
            if record.get('institution_name') or record.get('institution') or parse_year(record) is not None or record.get('grade_value') or record.get('gpa')
        ]

        if reliable_records:
            highest_record = max(
                reliable_records,
                key=lambda record: (degree_rank(record.get('degree_name') or record.get('degree')), parse_year(record) or -1),
                default=None
            )
        else:
            # Conservative fallback
            non_unknown = [r for r in education_sorted if degree_rank(r.get('degree_name') or r.get('degree')) > 0]
            highest_record = min(non_unknown, key=lambda record: degree_rank(record.get('degree_name') or record.get('degree')), default=None)
        ...
        summary = {
            ...
            "highest_qualification": (highest_record.get('degree_name') or highest_record.get('degree', 'N/A')) if highest_record else "N/A"
        }
    ...
    def analyze_professional_experience(self):
        ...
        for i in range(1, len(experience_sorted)):
            ...
            if current_start < previous_end:
                current_title = current.get('job_title') or 'current role'
                previous_title = previous.get('job_title') or 'previous role'
                overlaps.append(
                    f"Overlap detected between {previous_title} and {current_title}."
                )
            # Check for gaps
            ...
            if gap_days > 90: # More than 3 months
                ...
                previous_title = previous.get('job_title') or 'previous role'
                gaps.append(
                    f"A gap of ~{round(gap_days/30)} months found between {previous_title} and {current_title}."
                )
    ...
    def analyze_skill_alignment(self):
        ...
        """
        # Normalize skills - handle both strings and dicts
        skills = []
        ...
        for exp in self.data.get('experience', []):
            evidence_text += (exp.get('job_title', '') or '').lower() + " "
            evidence_text += (exp.get('key_responsibilities') or exp.get('job_description', '') or '').lower() + " "
            evidence_text += (exp.get('industry', '') or '').lower() + " "
        for pub in self.data.get('research_outputs', []):
            evidence_text += (pub.get('title', '') or '').lower() + " "
            evidence_text += (pub.get('venue', '') or pub.get('publication', '') or '').lower() + " "

        skill_aliases = {
            'aws': ['cloud', 'infrastructure', 'amazon web services', 'aws'],
            'docker': ['container', 'containers', 'orchestration', 'docker'],
            'machine learning': ['machine learning', 'ml', 'predictive model', 'predictive models', 'ai'],
            ...
            'sql': ['sql', 'database', 'databases', 'postgres', 'mysql'],
    ...
    def detect_missing_information(self):
        ...
        elif any((record.get('grade_value') or record.get('gpa')) in (None, '') for record in education):
            missing_fields.append("Grade/CGPA for one or more degrees")

        experience = list(self.data.get('experience', []))
        if not experience:
            missing_fields.append("Professional experience records")
        elif any(not (record.get('job_description') or record.get('key_responsibilities')) for record in experience):
